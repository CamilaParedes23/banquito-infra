# 🚀 Guía Rápida de Comandos - Banquito Infra
...
3. Click "Run workflow" → Selecciona servicio → Espera
```
...
bash scripts/deploy-core.sh api-gateway      # Solo API Gateway
...
# Solo específicos
docker-compose up -d core-ventanilla core-api-gateway sw-api-gateway
...
### Sin hacer Pull (más rápido)
```bash
bash scripts/restart-core.sh all
bash scripts/restart-core.sh ventanilla
bash scripts/restart-switch.sh all
bash scripts/restart-switch.sh lotes
```
...
cd ../switch
docker-compose restart sw-api-gateway
```
...
```bash
# En vivo
docker-compose logs -f core-ventanilla

# Últimas 50 líneas
docker-compose logs --tail 50 core-api-gateway
...
docker stats banquito-core-ventanilla
...
docker-compose up -d    # Se recrea automáticamente
...
### URLs Disponibles (después de configurar /etc/hosts)

```
CORE:
  http://core-api.banquito.local       → API Gateway (8090)
  http://api.banquito.local            → API Gateway (8090)
  http://core-web.banquito.local       → Banca Web (3000)
  http://web.banquito.local            → Banca Web (3000)
  http://core-ventanilla.banquito.local → Ventanilla Frontend (3001)
  http://ventanilla.banquito.local     → Ventanilla Frontend (3001)

SWITCH:
  http://switch-api.banquito.local     → API Gateway (8080)
  http://switch-web.banquito.local     → Switch Frontend (3002)
```
...
# Todo: imágenes, volúmenes, redes no usadas
...
bdeploy-core ventanilla   # Deploy rápido
